...
for (no_repeats in c(50, 500, 1000)) {
	...
	plot(random_values1, rep(0, no_values1), main = "Rozmieszczenie punktów z pierwszego generatora na odcinku", xlab = "wartość z odcinka", ylab = "")
	abline(h = 0)
	dev.off()

	png(paste("generator1hist_", no_repeats, ".png", sep = ""))
	hist(random_values1, breaks = 10, probability = TRUE, main = "Histogram pierwszego generatora", xlab = "zakresy", ylab = "prawdopodobieństwo")
	abline(h = 1, col = "green", lwd = 3)
	dev.off()

	png(paste("generator2line_", no_repeats, ".png", sep = ""))
	plot(random_values2, rep(0, no_values2), main = "Rozmieszczenie punktów z drugiego generatora na odcinku", xlab = "wartość z odcinka", ylab = "")
	abline(h = 0)
	dev.off()

	png(paste("generator2hist_", no_repeats, ".png", sep = ""))
	hist(random_values2, breaks = 10, probability = TRUE, main = "Histogram drugiego generatora", xlab = "zakresy", ylab = "prawdopodobieństwo")
	abline(h = 1, col = "green", lwd = 3)
	dev.off()
	...
	plot(random_values1[2 * 1:(no_values1 / 2) - 1], random_values1[2 * 1:(no_values1 / 2)], main = "Rozmieszczenie punktów z pierwszego generatora na kwadracie", xlab = "pierwsza współrzędna", ylab = "druga współrzędna")
	dev.off()

	png(paste("dependent2_", no_repeats, ".png", sep = ""))
	plot(random_values2[2 * 1:(no_values2 / 2) - 1], random_values2[2 * 1:(no_values2 / 2)], main = "Rozmieszczenie punktów z drugiego generatora na kwadracie", xlab = "pierwsza współrzędna", ylab = "druga współrzędna")
	dev.off()
